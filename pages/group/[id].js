...
const isUserInGroup = async () => {
  try {
    ...
    const nongroup = await axios.post('http://129.151.255.136:3001/non_group_users', {
...
const handleCreateTask = async (e) => {
  ...
  if (!desc_task || !assignedUser || !assignedUserPerm || !deadline) {
    ...
    try {
      const token = await getToken();
      await axios.post('http://129.151.255.136:3001/create_task', {
        id_group: group.id_group,
...
const handleEditTask = async (e) => {
  ...
  try {
    const token = await getToken();
    await axios.post(
      'http://129.151.255.136:3001/update_task_info',
      {
...
const setAsdoneTask = async ( selectedtask ) => {
  try {
    const token = await getToken();
    const response = await axios.post('http://129.151.255.136:3001/update_task_state', {
      id_task: selectedTask.task.id_task,
      state_task: 2,
    },
     {
...
const addUserToGroup = async (user) => {
  try {
    ...
    await axios.post(
      'http://129.151.255.136:3001/add_user_to_group',
      {
...
//
 //users que não estão no grupo
  const fetchNonGroupUsers = async () => {
    try {
      const token = await getToken();
      const response = await axios.post('http://129.151.255.136:3001/non_group_users', {
        id_group: group.id_group
      }, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
// users do grupo
  const fetchGroupUsers = async () => {
    try {
      const token = await getToken();
      const response = await axios.post('http://129.151.255.136:3001/group_users', {
      id_group: group.id_group
      }, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
useEffect(() => {
  const fetchTasksByGroup = async () => {
    try {
      const res = await fetch("http://129.151.255.136:3001/tasks_by_group", {
        method: "POST",
        headers: {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await instance_file.post(
        `http://129.151.255.136:3001/upload_file?id_group=${group.id_group}`,
        formData,
        {
          headers: {
...
const handleFilesListPopUp = async () => {
  ...
  try {
    const response = await axios.post("http://129.151.255.136:3001/files_list", { id_group: group.id_group }, {
...
const handleDelete = async (fileName) => {
  ...
  try {
    const response = await instance_delete.post(
      'http://129.151.255.136:3001/delete_file',
      {
        id_group: group.id_group,
        file_name: fileName
      }
        ...
        <div>
          {files.map((file) => (
            <li style={{ listStyle: 'none' , margintop : "20px"  }} key={file.name}>
              <a  style={{ marginRight : "8px" }}href={`http://129.151.255.136:3001/download_file?id_group=${group.id_group}&file_name=${file.name}`}>
                {file.name}
              </a>
...
export async function getServerSideProps({ params, req }) {
  ...
  try {
    const response = await axios.get(`http://129.151.255.136:3001/group/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
